...
- (void)viewDidLoad {
    ...
    [self testNSDateTimeAgo];
}

- (void)testNSDateTimeAgo
{
    NSDate *date = [[NSDate alloc] initWithTimeIntervalSince1970:0];
    NSString *ago = [date timeAgo];
    NSLog(@"Output is: \"%@\"", ago);
    NSDate *now = [[NSDate alloc]initWithTimeIntervalSince1970:0];
    NSLog(@"%@",[now timeAgoSimple]);
    NSLog(@"%@",[now timeAgo]);
    NSLog(@"%@",[now dateTimeAgo]);
    NSLog(@"%@",[now dateTimeUntilNow]);
