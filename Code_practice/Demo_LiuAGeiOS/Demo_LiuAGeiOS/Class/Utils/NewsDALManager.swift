    ...
    func loadNewsList(classid: Int, pageIndex: Int, type: Int, cache: Bool, finished: (result: JSON?, error: NSError?) -> ()) {
        ...
        if cache {
            ...
            loadNewsListFromLocation(classid, pageIndex: pageIndex, type: type) { (success, result, error) in
                ...
                // 本地没有数据才从网络中加载
                NetWorkTool.shareNetworkTool.loadNewsListFromNetwork(classid, pageIndex: pageIndex, type: type) { (success, result, error) in
    ...
    func loadNewsDetail(classid: Int, id: Int, cache: Bool, finished: (result: JSON?, error: NSError?) -> ()) {
        ...
        if cache {
            loadNewsDetailFromLocation(classid, id: id) { (success, result, error) in
                ...
                NetWorkTool.shareNetworkTool.loadNewsDetailFromNetwork(classid, id: id, finished: { (success, result, error) in
...
extension NewsDALManager {
    ...
    func loadCommentList(classid: Int, id: Int, pageIndex: Int, pageSize: Int, finished: (result: JSON?, error: NSError?) -> ()) {
        ...
        // 评论不做数据缓存，直接从网络请求
        NetWorkTool.shareNetworkTool.loadCommentListFromNetwork(classid, id: id, pageIndex: pageIndex, pageSize: pageSize) { (success, result, error) in
